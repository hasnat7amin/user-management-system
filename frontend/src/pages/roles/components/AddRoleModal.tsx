...
interface AddRoleModalProps {
    ...
    isLoading: boolean;
    onClose: () => void;
    onSubmit: (values: { name: string; permissions: string[] }, onSuccess: () => void) => void;
}

const AddRoleModal: React.FC<AddRoleModalProps> = ({ isOpen, isLoading, onClose, onSubmit }) => {
    ...
    return (
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50">
            <div className="bg-gray-900 text-white p-6 rounded-lg shadow-lg w-full max-w-md">
                ...
                {/* Form */}
                <Formik
                    initialValues={{ name: "", permissions: selectedPermissions }}
                    enableReinitialize
                    validationSchema={validationSchema}
                    onSubmit={(values) => onSubmit({ ...values, permissions: selectedPermissions }, onClose)}
                >
                    {() => (
                            ...
                            <div>
                                ...
                                {isPermissionsLoading ? (
                                    ...
                                    <div className="flex flex-wrap gap-2 mt-2">
                                        {permissions.map((permission: { _id: string; name: string }) => (
                                            <span
                                                key={permission._id}
